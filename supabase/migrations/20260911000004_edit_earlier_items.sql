-- Allow fixing one earlier item without wiping later work.
-- Going Live still requires every earlier item to be complete.

CREATE OR REPLACE FUNCTION public.has_incomplete_items_before(
  p_agent_id uuid,
  p_before_sort_order int
)
RETURNS boolean
LANGUAGE sql
STABLE
...
AS $$
  SELECT EXISTS (
    SELECT 1
    FROM public.agent_substeps AS item
    JOIN public.agent_stages AS agent_stage
      ON agent_stage.id = item.agent_stage_id
    JOIN public.stages AS stage
      ON stage.id = agent_stage.stage_id
    WHERE agent_stage.agent_id = p_agent_id
      AND stage.sort_order < p_before_sort_order
...
BEGIN
  ...
  IF v_stage_status = 'complete' AND v_stage_id = v_current_stage_id THEN
  ...
  SET status = v_stage_status,
      ...
      END,
      actual_end = CASE
        WHEN v_stage_status = 'complete' THEN COALESCE(actual_end, CURRENT_DATE)
        ELSE NULL
      END
  WHERE id = v_agent_stage_id;

  UPDATE public.agents
  SET status = CASE WHEN status = 'complete' THEN 'active' ELSE status END
  WHERE id = v_agent_id;
...
BEGIN
  ...
  SELECT agent_stage.stage_id, stage.sort_order, stage.name
  INTO v_stage_id, v_stage_sort_order, v_stage_name
  ...
  SELECT next_agent_stage.id, next_agent_stage.stage_id, next_stage.name, next_stage.sort_order
  INTO v_next_agent_stage_id, v_next_stage_id, v_next_stage_name, v_next_stage_sort_order
  ...
  IF (
    v_next_stage_name = 'Live'
    AND public.has_incomplete_items_before(p_agent_id, v_next_stage_sort_order)
  ) OR (
    v_next_agent_stage_id IS NULL
    AND v_stage_name = 'Live'
    AND public.has_incomplete_items_before(p_agent_id, v_stage_sort_order)
  ) THEN
    RAISE EXCEPTION 'Finish every item in earlier stages before this request can go Live.';
  END IF;
...
REVOKE ALL ON FUNCTION public.has_incomplete_items_before(uuid, int) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.has_incomplete_items_before(uuid, int) TO authenticated;
