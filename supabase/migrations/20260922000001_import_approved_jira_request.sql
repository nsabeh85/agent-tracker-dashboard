-- AIBU-79: ingest approved PCT AI Requests as pending_approval tracker rows.
-- Dedupe on source_url / Jira key. Service role only; no frontend secret.

CREATE UNIQUE INDEX IF NOT EXISTS agents_source_url_unique
  ON public.agents (source_url)
  WHERE source_url IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS agents_jira_browse_key_unique
  ON public.agents (
    (upper(substring(source_url FROM 'PCT-[0-9]+$')))
  )
  WHERE source_url ~* '/browse/PCT-[0-9]+$';
...
REVOKE ALL ON FUNCTION public.import_approved_jira_request(
